...
const whatsappAriaLabel = `Chat with ${brand.name}`;

type ButtonProps = {
  className?: string;
};

/** Primary CTA — enterprise wording, opens WhatsApp with prefilled message. */
export function WhatsAppConnectButton({ className = "" }: ButtonProps) {
  return (
    <a
      href={whatsappHref()}
      target="_blank"
...
/** Header — icon-only ghost control, no “WhatsApp” label. */
export function WhatsAppHeaderButton({ className = "" }: ButtonProps) {
  return (
    <a
      href={whatsappHref()}
      target="_blank"
...
/** Icon-only link for footer nav row. */
...
/** Floating action — icon only, subtle premium pulse. */
...
/** @deprecated Use WhatsAppIconLink */
export function WhatsAppTextLink({ className = "" }: { className?: string }) {
  return <WhatsAppIconLink className={className} />;
}
